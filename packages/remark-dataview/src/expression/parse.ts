...
import {
  ...
  TagSource,
  CsvSource,
} from "../data-index/source.js";
import { normalizeDuration } from "../util/normalize.js";
...
export const EXPRESSION = P.createLanguage<ExpressionLanguage>({
  ...
  identifier: (_) =>
    P.seqMap(
      ...
      P.alt(
        P.regexp(/[0-9\p{Letter}_-]/u),
        P.regexp(EMOJI_REGEX).desc("text")
      ).many(),
  ...
  csvSource: (q) =>
    P.seqMap(
      P.string("csv(").skip(P.optWhitespace),
      q.string,
      P.string(")"),
  ...
  negateSource: (q) =>
    P.seqMap(P.alt(P.string("-"), P.string("!")), q.atomSource, (_, source) =>
      Sources.negate(source)
    ),
  atomSource: (q) =>
    P.alt<Source>(
      q.parensSource,
